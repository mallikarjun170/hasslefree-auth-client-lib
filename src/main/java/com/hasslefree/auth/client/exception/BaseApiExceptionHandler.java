...
public abstract class BaseApiExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleMethodNotAllowed(HttpRequestMethodNotSupportedException ex,
        ...
        if (supportedHttpMethods != null && !supportedHttpMethods.isEmpty()) {
            supportedMethods = String.join(", ", supportedHttpMethods.stream().map(HttpMethod::name).toList());
        } else {
    ...
    public ResponseEntity<ApiErrorResponse> handleValidationExceptions(MethodArgumentNotValidException ex,
        ...
        return buildErrorResponse(HttpStatus.BAD_REQUEST, "Validation Failed", "Invalid input parameters", request, errors);
    }
    ...
    public ResponseEntity<ApiErrorResponse> handleInternalError(Exception ex, HttpServletRequest request) {
        ...
        ResponseEntity<ApiErrorResponse> response = buildErrorResponse(HttpStatus.INTERNAL_SERVER_ERROR,
                "Internal Server Error", "An unexpected error occurred", request, null);
        MDC.remove("errorId");
        return response;
    }

    protected ResponseEntity<ApiErrorResponse> buildErrorResponse(HttpStatus status, String error, String message,
    ...
    protected String sanitizeUri(String description) {
        ...
        return description.replaceAll("(?i)(token|password|secret|key|authorization)=[^;\\s&]*", "$1=***");
    }
